...
class SequenceModule(SequenceFields, ProctoringFields, XModule):
        ...
        try:
            ...
            for block in traverse_pre_order(item, get_children, leaf_filter):
                ...
                if gate_fragment is not None:
                    # This variable is used for gating timed exams that contain content type gated content
                    self.gated_sequence_paywall = gate_fragment.content
                    return gate_fragment
    ...
    def gate_entire_sequence_if_it_is_a_timed_exam_and_contains_content_type_gated_problems(self):
        ...
        """
        if self.is_time_limited and self.gated_sequence_paywall:
            return self.gated_sequence_paywall
        return None
    ...
    def _special_exam_student_view(self):
        ...
        """
        _ = self.runtime.service(self, "i18n").ugettext
        if self.is_time_limited and not self.gated_sequence_paywall:
    ...
    def _get_render_metadata(self, context, display_items, prereq_met, prereq_meta_info, banner_text=None, view=STUDENT_VIEW, fragment=None):
        ...
        if TIMED_EXAM_GATING_WAFFLE_FLAG.is_enabled():
            if self.gate_entire_sequence_if_it_is_a_timed_exam_and_contains_content_type_gated_problems():
                params['gated_sequence_paywall'] = self.gated_sequence_paywall
